...
#ifndef MODM_STM32_GPIO_PIN_C13_HPP
#define MODM_STM32_GPIO_PIN_C13_HPP
...
class GpioC13 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioC13>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioC13;
	using Input = GpioC13;
	using IO = GpioC13;
	using Type = GpioC13;
...
public:
	...
	/// Connect to Rtc
	using OutAlarm = GpioSignal;
	/// Connect to Rtc
	using OutCalib = GpioSignal;
	/// Connect to Rtc
	using Tamp1 = GpioSignal;
	/// Connect to Rtc
	using Ts = GpioSignal;
	/// Connect to Sys
	using Wkup2 = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioC13::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct OutAlarm { static void connect();
		static_assert(
			(peripheral == Peripheral::Rtc),
			"GpioC13::OutAlarm only connects to Rtc!");
	};
	template< Peripheral peripheral >
	struct OutCalib { static void connect();
		static_assert(
			(peripheral == Peripheral::Rtc),
			"GpioC13::OutCalib only connects to Rtc!");
	};
	template< Peripheral peripheral >
	struct Tamp1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Rtc),
			"GpioC13::Tamp1 only connects to Rtc!");
	};
	template< Peripheral peripheral >
	struct Ts { static void connect();
		static_assert(
			(peripheral == Peripheral::Rtc),
			"GpioC13::Ts only connects to Rtc!");
	};
	template< Peripheral peripheral >
	struct Wkup2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Sys),
			"GpioC13::Wkup2 only connects to Sys!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioC13::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioC13;
...
template<>
struct GpioC13::OutAlarm<Peripheral::Rtc>
{
	using Gpio = GpioC13;
...
template<>
struct GpioC13::OutCalib<Peripheral::Rtc>
{
	using Gpio = GpioC13;
...
template<>
struct GpioC13::Tamp1<Peripheral::Rtc>
{
	using Gpio = GpioC13;
...
template<>
struct GpioC13::Ts<Peripheral::Rtc>
{
	using Gpio = GpioC13;
...
template<>
struct GpioC13::Wkup2<Peripheral::Sys>
{
	using Gpio = GpioC13;
...
#endif // MODM_STM32_GPIO_PIN_C13_HPP