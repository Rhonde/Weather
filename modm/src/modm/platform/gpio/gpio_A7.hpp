...
public:
	...
	struct In12 { static void connect();
		static_assert(
			...
			"GpioA7::In12 only connects to Adc1 or Adc2!");
	};
	template< Peripheral peripheral >
	...
	template< Peripheral peripheral >
	struct Seg4 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioA7::Seg4 only connects to Lcd!");
	};
	template< Peripheral peripheral >
	struct Vinm { static void connect();
		static_assert(
			(peripheral == Peripheral::Opamp2),
			"GpioA7::Vinm only connects to Opamp2!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioA7::In12<Peripheral::Adc2>
{
	using Gpio = GpioA7;
	static constexpr Gpio::Signal Signal = Gpio::Signal::In12;
	static constexpr int af = -1;
...
template<>
constexpr int8_t
GpioA7::AdcChannel<Peripheral::Adc2> = 12;
template<>
...
template<>
struct GpioA7::Seg4<Peripheral::Lcd>
{
	using Gpio = GpioA7;
...
template<>
struct GpioA7::Vinm<Peripheral::Opamp2>
{
	using Gpio = GpioA7;
