...
#ifndef MODM_STM32_GPIO_PIN_C3_HPP
#define MODM_STM32_GPIO_PIN_C3_HPP
...
class GpioC3 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioC3>;
	friend class Adc;
...
public:
	...
	struct Etr { static void connect();
		static_assert(
			...
			"GpioC3::Etr only connects to Lptim1 or Lptim2!");
	};
	template< Peripheral peripheral >
	struct In4 { static void connect();
		static_assert(
			...
			"GpioC3::In4 only connects to Adc1 or Adc2 or Adc3!");
	};
	template< Peripheral peripheral >
	...
	template< Peripheral peripheral >
	struct Vlcd { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioC3::Vlcd only connects to Lcd!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioC3::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioC3;
...
template<>
struct GpioC3::Etr<Peripheral::Lptim2>
{
	using Gpio = GpioC3;
	static constexpr Gpio::Signal Signal = Gpio::Signal::Etr;
...
template<>
struct GpioC3::In4<Peripheral::Adc1>
{
	using Gpio = GpioC3;
...
template<>
constexpr int8_t
GpioC3::AdcChannel<Peripheral::Adc1> = 4;
template<>
struct GpioC3::In4<Peripheral::Adc2>
{
	using Gpio = GpioC3;
	static constexpr Gpio::Signal Signal = Gpio::Signal::In4;
	static constexpr int af = -1;
...
template<>
constexpr int8_t
GpioC3::AdcChannel<Peripheral::Adc2> = 4;
template<>
struct GpioC3::In4<Peripheral::Adc3>
{
	using Gpio = GpioC3;
	static constexpr Gpio::Signal Signal = Gpio::Signal::In4;
	static constexpr int af = -1;
...
template<>
constexpr int8_t
GpioC3::AdcChannel<Peripheral::Adc3> = 4;
template<>
...
template<>
struct GpioC3::Vlcd<Peripheral::Lcd>
{
	using Gpio = GpioC3;
...
#endif // MODM_STM32_GPIO_PIN_C3_HPP