...
public:
	...
	struct In8 { static void connect();
		static_assert(
			...
			"GpioA3::In8 only connects to Adc1 or Adc2!");
	};
	template< Peripheral peripheral >
	struct Rx { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart2),
			"GpioA3::Rx only connects to Usart2!");
	};
	template< Peripheral peripheral >
	struct Seg2 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioA3::Seg2 only connects to Lcd!");
	};
	template< Peripheral peripheral >
	struct Vout { static void connect();
		static_assert(
			(peripheral == Peripheral::Opamp1),
			"GpioA3::Vout only connects to Opamp1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioA3::In8<Peripheral::Adc1>
{
	using Gpio = GpioA3;
...
template<>
constexpr int8_t
GpioA3::AdcChannel<Peripheral::Adc1> = 8;
template<>
struct GpioA3::In8<Peripheral::Adc2>
{
	using Gpio = GpioA3;
	static constexpr Gpio::Signal Signal = Gpio::Signal::In8;
	static constexpr int af = -1;
...
template<>
constexpr int8_t
GpioA3::AdcChannel<Peripheral::Adc2> = 8;
template<>
...
template<>
struct GpioA3::Seg2<Peripheral::Lcd>
{
	using Gpio = GpioA3;
...
template<>
struct GpioA3::Vout<Peripheral::Opamp1>
{
	using Gpio = GpioA3;
