...
public:
	...
	struct In13 { static void connect();
		static_assert(
			...
			"GpioC4::In13 only connects to Adc1 or Adc2!");
	};
	template< Peripheral peripheral >
	struct Inm { static void connect();
		static_assert(
			(peripheral == Peripheral::Comp1),
			"GpioC4::Inm only connects to Comp1!");
	};
	template< Peripheral peripheral >
	struct Seg22 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioC4::Seg22 only connects to Lcd!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart3),
			"GpioC4::Tx only connects to Usart3!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioC4::In13<Peripheral::Adc1>
{
	using Gpio = GpioC4;
...
template<>
constexpr int8_t
GpioC4::AdcChannel<Peripheral::Adc1> = 13;
template<>
struct GpioC4::In13<Peripheral::Adc2>
{
	using Gpio = GpioC4;
	static constexpr Gpio::Signal Signal = Gpio::Signal::In13;
	static constexpr int af = -1;
...
template<>
constexpr int8_t
GpioC4::AdcChannel<Peripheral::Adc2> = 13;
template<>
struct GpioC4::Inm<Peripheral::Comp1>
{
	using Gpio = GpioC4;
...
template<>
struct GpioC4::Seg22<Peripheral::Lcd>
{
	using Gpio = GpioC4;
...
template<>
struct GpioC4::Tx<Peripheral::Usart3>
{
	using Gpio = GpioC4;
