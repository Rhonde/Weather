...
#ifndef MODM_STM32_GPIO_PIN_C11_HPP
#define MODM_STM32_GPIO_PIN_C11_HPP
...
class GpioC11 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioC11>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioC11;
	using Input = GpioC11;
	using IO = GpioC11;
	using Type = GpioC11;
...
public:
	...
	/// Connect to Lcd
	using Com5 = GpioSignal;
	/// Connect to Sdmmc1
	using D3 = GpioSignal;
	/// Connect to Tsc
	using G3Io3 = GpioSignal;
	/// Connect to Sai2
	...
	using Rx = GpioSignal;
	/// Connect to Lcd
	using Seg29 = GpioSignal;
	/// Connect to Lcd
	using Seg41 = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	template< Peripheral peripheral >
	struct Com5 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioC11::Com5 only connects to Lcd!");
	};
	template< Peripheral peripheral >
	struct D3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Sdmmc1),
			"GpioC11::D3 only connects to Sdmmc1!");
	};
	template< Peripheral peripheral >
	struct G3Io3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tsc),
			"GpioC11::G3Io3 only connects to Tsc!");
	};
	template< Peripheral peripheral >
	struct MclkB { static void connect();
		static_assert(
			(peripheral == Peripheral::Sai2),
			"GpioC11::MclkB only connects to Sai2!");
	};
	template< Peripheral peripheral >
	struct Miso { static void connect();
		static_assert(
			(peripheral == Peripheral::Spi3),
			"GpioC11::Miso only connects to Spi3!");
	};
	template< Peripheral peripheral >
	...
	template< Peripheral peripheral >
	struct Seg29 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioC11::Seg29 only connects to Lcd!");
	};
	template< Peripheral peripheral >
	struct Seg41 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioC11::Seg41 only connects to Lcd!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioC11::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioC11;
...
template<>
struct GpioC11::Com5<Peripheral::Lcd>
{
	using Gpio = GpioC11;
...
template<>
struct GpioC11::D3<Peripheral::Sdmmc1>
{
	using Gpio = GpioC11;
...
template<>
struct GpioC11::G3Io3<Peripheral::Tsc>
{
	using Gpio = GpioC11;
...
template<>
struct GpioC11::MclkB<Peripheral::Sai2>
{
	using Gpio = GpioC11;
...
template<>
struct GpioC11::Miso<Peripheral::Spi3>
{
	using Gpio = GpioC11;
...
template<>
struct GpioC11::Rx<Peripheral::Usart3>
{
	using Gpio = GpioC11;
...
template<>
struct GpioC11::Rx<Peripheral::Uart4>
{
	using Gpio = GpioC11;
	static constexpr Gpio::Signal Signal = Gpio::Signal::Rx;
...
template<>
struct GpioC11::Seg29<Peripheral::Lcd>
{
	using Gpio = GpioC11;
...
template<>
struct GpioC11::Seg41<Peripheral::Lcd>
{
	using Gpio = GpioC11;
...
#endif // MODM_STM32_GPIO_PIN_C11_HPP