...
public:
	...
	struct In6 { static void connect();
		static_assert(
			...
			"GpioA1::In6 only connects to Adc1 or Adc2!");
	};
	template< Peripheral peripheral >
	...
	template< Peripheral peripheral >
	struct Seg0 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioA1::Seg0 only connects to Lcd!");
	};
	template< Peripheral peripheral >
	struct Vinm { static void connect();
		static_assert(
			(peripheral == Peripheral::Opamp1),
			"GpioA1::Vinm only connects to Opamp1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioA1::In6<Peripheral::Adc1>
{
	using Gpio = GpioA1;
...
template<>
constexpr int8_t
GpioA1::AdcChannel<Peripheral::Adc1> = 6;
template<>
struct GpioA1::In6<Peripheral::Adc2>
{
	using Gpio = GpioA1;
	static constexpr Gpio::Signal Signal = Gpio::Signal::In6;
	static constexpr int af = -1;
...
template<>
constexpr int8_t
GpioA1::AdcChannel<Peripheral::Adc2> = 6;
template<>
...
template<>
struct GpioA1::Seg0<Peripheral::Lcd>
{
	using Gpio = GpioA1;
...
template<>
struct GpioA1::Vinm<Peripheral::Opamp1>
{
	using Gpio = GpioA1;
