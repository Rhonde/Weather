...
#ifndef MODM_STM32_GPIO_PIN_A12_HPP
#define MODM_STM32_GPIO_PIN_A12_HPP
...
class GpioA12 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioA12>;
	friend class Adc;
	friend class Adc1; friend class Adc2;
	friend class Adc3; friend class Adc4;
public:
	using Output = GpioA12;
	using Input = GpioA12;
	using IO = GpioA12;
	using Type = GpioA12;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioA12::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct De { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart1),
			"GpioA12::De only connects to Usart1!");
	};
	template< Peripheral peripheral >
	...
	struct Etr { static void connect();
		static_assert(
			...
			"GpioA12::Etr only connects to Tim1!");
	};
	template< Peripheral peripheral >
	struct Rts { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart1),
			"GpioA12::Rts only connects to Usart1!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Can1),
			"GpioA12::Tx only connects to Can1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioA12::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioA12;
...
template<>
struct GpioA12::De<Peripheral::Usart1>
{
	using Gpio = GpioA12;
...
template<>
struct GpioA12::Etr<Peripheral::Tim1>
{
	using Gpio = GpioA12;
...
template<>
struct GpioA12::Rts<Peripheral::Usart1>
{
	using Gpio = GpioA12;
...
template<>
struct GpioA12::Tx<Peripheral::Can1>
{
	using Gpio = GpioA12;
...
#endif // MODM_STM32_GPIO_PIN_A12_HPP