...
public:
	...
	struct Cts { static void connect();
		static_assert(
			...
			"GpioB4::Cts only connects to Usart1!");
	};
	template< Peripheral peripheral >
	...
	template< Peripheral peripheral >
	struct G2Io1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tsc),
			"GpioB4::G2Io1 only connects to Tsc!");
	};
	template< Peripheral peripheral >
	struct Inp { static void connect();
		static_assert(
			(peripheral == Peripheral::Comp2),
			"GpioB4::Inp only connects to Comp2!");
	};
	template< Peripheral peripheral >
	...
	template< Peripheral peripheral >
	struct Seg8 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioB4::Seg8 only connects to Lcd!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioB4::Cts<Peripheral::Usart1>
{
	using Gpio = GpioB4;
...
template<>
struct GpioB4::G2Io1<Peripheral::Tsc>
{
	using Gpio = GpioB4;
...
template<>
struct GpioB4::Inp<Peripheral::Comp2>
{
	using Gpio = GpioB4;
...
template<>
struct GpioB4::Seg8<Peripheral::Lcd>
{
	using Gpio = GpioB4;
