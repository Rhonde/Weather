...
public:
	...
	using D0 = GpioSignal;
	/// Connect to Tsc
	using G4Io3 = GpioSignal;
	/// Connect to Lcd
	using Seg26 = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	struct D0 { static void connect();
		static_assert(
			...
			"GpioC8::D0 only connects to Sdmmc1!");
	};
	template< Peripheral peripheral >
	struct G4Io3 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tsc),
			"GpioC8::G4Io3 only connects to Tsc!");
	};
	template< Peripheral peripheral >
	struct Seg26 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioC8::Seg26 only connects to Lcd!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
template<>
struct GpioC8::D0<Peripheral::Sdmmc1>
{
	using Gpio = GpioC8;
...
template<>
struct GpioC8::G4Io3<Peripheral::Tsc>
{
	using Gpio = GpioC8;
...
template<>
struct GpioC8::Seg26<Peripheral::Lcd>
{
	using Gpio = GpioC8;
