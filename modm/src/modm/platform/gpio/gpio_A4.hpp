...
#ifndef MODM_STM32_GPIO_PIN_A4_HPP
#define MODM_STM32_GPIO_PIN_A4_HPP
...
class GpioA4 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioA4>;
	friend class Adc;
...
public:
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioA4::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ck { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart2),
			"GpioA4::Ck only connects to Usart2!");
	};
	template< Peripheral peripheral >
	struct FsB { static void connect();
		static_assert(
			(peripheral == Peripheral::Sai1),
			"GpioA4::FsB only connects to Sai1!");
	};
	template< Peripheral peripheral >
	struct In9 { static void connect();
		static_assert(
			...
			"GpioA4::In9 only connects to Adc1 or Adc2!");
	};
	template< Peripheral peripheral >
	...
	struct Out { static void connect();
		static_assert(
			...
			"GpioA4::Out only connects to Lptim2!");
	};
	template< Peripheral peripheral >
	struct Out1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Dac1),
			"GpioA4::Out1 only connects to Dac1!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioA4::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioA4;
...
template<>
struct GpioA4::Ck<Peripheral::Usart2>
{
	using Gpio = GpioA4;
...
template<>
struct GpioA4::FsB<Peripheral::Sai1>
{
	using Gpio = GpioA4;
...
template<>
struct GpioA4::In9<Peripheral::Adc1>
{
	using Gpio = GpioA4;
...
template<>
constexpr int8_t
GpioA4::AdcChannel<Peripheral::Adc1> = 9;
template<>
struct GpioA4::In9<Peripheral::Adc2>
{
	using Gpio = GpioA4;
	static constexpr Gpio::Signal Signal = Gpio::Signal::In9;
	static constexpr int af = -1;
...
template<>
constexpr int8_t
GpioA4::AdcChannel<Peripheral::Adc2> = 9;
template<>
struct GpioA4::Nss<Peripheral::Spi1>
{
	using Gpio = GpioA4;
...
template<>
struct GpioA4::Nss<Peripheral::Spi3>
{
	using Gpio = GpioA4;
	static constexpr Gpio::Signal Signal = Gpio::Signal::Nss;
...
template<>
struct GpioA4::Out<Peripheral::Lptim2>
{
	using Gpio = GpioA4;
...
template<>
struct GpioA4::Out1<Peripheral::Dac1>
{
	using Gpio = GpioA4;
...
#endif // MODM_STM32_GPIO_PIN_A4_HPP