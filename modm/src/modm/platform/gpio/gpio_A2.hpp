...
#ifndef MODM_STM32_GPIO_PIN_A2_HPP
#define MODM_STM32_GPIO_PIN_A2_HPP
...
class GpioA2 : public Gpio, public ::modm::GpioIO
{
	template<class... Gpios>
	friend class GpioSet;
	using PinSet = GpioSet<GpioA2>;
	friend class Adc;
...
public:
	...
	/// Connect to Rcc
	using Lsco = GpioSignal;
	/// Connect to Lcd
	using Seg1 = GpioSignal;
	/// Connect to Usart2
	using Tx = GpioSignal;
	/// Connect to Sys
	using Wkup4 = GpioSignal;
	/// @}
#endif
	/// @cond
	...
	struct BitBang { static void connect();
		static_assert(
			...
			"GpioA2::BitBang only connects to software drivers!");
	};
	template< Peripheral peripheral >
	struct Ch1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Tim15),
			"GpioA2::Ch1 only connects to Tim15!");
	};
	template< Peripheral peripheral >
	...
	struct Extclk { static void connect();
		static_assert(
			...
			"GpioA2::Extclk only connects to Sai2!");
	};
	template< Peripheral peripheral >
	struct In7 { static void connect();
		static_assert(
			...
			"GpioA2::In7 only connects to Adc1 or Adc2!");
	};
	template< Peripheral peripheral >
	struct Lsco { static void connect();
		static_assert(
			(peripheral == Peripheral::Rcc),
			"GpioA2::Lsco only connects to Rcc!");
	};
	template< Peripheral peripheral >
	struct Seg1 { static void connect();
		static_assert(
			(peripheral == Peripheral::Lcd),
			"GpioA2::Seg1 only connects to Lcd!");
	};
	template< Peripheral peripheral >
	struct Tx { static void connect();
		static_assert(
			(peripheral == Peripheral::Usart2),
			"GpioA2::Tx only connects to Usart2!");
	};
	template< Peripheral peripheral >
	struct Wkup4 { static void connect();
		static_assert(
			(peripheral == Peripheral::Sys),
			"GpioA2::Wkup4 only connects to Sys!");
	};
	/// @endcond
private:
	template< Peripheral peripheral >
...
/// @cond
template<>
struct GpioA2::BitBang<Peripheral::BitBang>
{
	using Gpio = GpioA2;
...
template<>
struct GpioA2::Ch1<Peripheral::Tim15>
{
	using Gpio = GpioA2;
...
template<>
struct GpioA2::Ch3<Peripheral::Tim2>
{
	using Gpio = GpioA2;
...
template<>
struct GpioA2::Ch3<Peripheral::Tim5>
{
	using Gpio = GpioA2;
	static constexpr Gpio::Signal Signal = Gpio::Signal::Ch3;
...
template<>
struct GpioA2::Extclk<Peripheral::Sai2>
{
	using Gpio = GpioA2;
...
template<>
struct GpioA2::In7<Peripheral::Adc1>
{
	using Gpio = GpioA2;
...
template<>
constexpr int8_t
GpioA2::AdcChannel<Peripheral::Adc1> = 7;
template<>
struct GpioA2::In7<Peripheral::Adc2>
{
	using Gpio = GpioA2;
	static constexpr Gpio::Signal Signal = Gpio::Signal::In7;
	static constexpr int af = -1;
...
template<>
constexpr int8_t
GpioA2::AdcChannel<Peripheral::Adc2> = 7;
template<>
struct GpioA2::Lsco<Peripheral::Rcc>
{
	using Gpio = GpioA2;
...
template<>
struct GpioA2::Seg1<Peripheral::Lcd>
{
	using Gpio = GpioA2;
...
template<>
struct GpioA2::Tx<Peripheral::Usart2>
{
	using Gpio = GpioA2;
...
template<>
struct GpioA2::Wkup4<Peripheral::Sys>
{
	using Gpio = GpioA2;
...
#endif // MODM_STM32_GPIO_PIN_A2_HPP