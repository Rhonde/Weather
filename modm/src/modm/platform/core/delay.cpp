...
_delay_ns(uint16_t ns)
{
	...
	:: "r" (ns), "r" (modm::clock::ns_per_loop), "r" (10));
...
_delay_us(uint16_t us)
{
	...
	int32_t delay = int32_t(modm::clock::fcpu_MHz) * us - 30;
...
_delay_ms(uint16_t ms)
{
	...
	uint32_t start = DWT->CYCCNT;
	int32_t delay = int32_t(modm::clock::fcpu_kHz) * ms - 30;
...
modm_section(".hardware_init.A000100_modm_dwt_enable")
uint32_t modm_dwt_enable_ptr = (uint32_t) &modm_dwt_enable;
