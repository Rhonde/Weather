...
Rcc::enable()
{
	...
	if constexpr (peripheral == Peripheral::Lcd)
		if (not Rcc::isEnabled<peripheral>()) {
			RCC->APB1ENR1 |= RCC_APB1ENR1_LCDEN; __DSB();
			RCC->APB1RSTR1 |= RCC_APB1RSTR1_LCDRST; __DSB();
	...
	if constexpr (peripheral == Peripheral::Sai2)
		if (not Rcc::isEnabled<peripheral>()) {
			RCC->APB2ENR |= RCC_APB2ENR_SAI2EN; __DSB();
			RCC->APB2RSTR |= RCC_APB2RSTR_SAI2RST; __DSB();
			RCC->APB2RSTR &= ~RCC_APB2RSTR_SAI2RST;
		}
	if constexpr (peripheral == Peripheral::Sdmmc1)
		if (not Rcc::isEnabled<peripheral>()) {
			RCC->APB2ENR |= RCC_APB2ENR_SDMMC1EN; __DSB();
			RCC->APB2RSTR |= RCC_APB2RSTR_SDMMC1RST; __DSB();
...
Rcc::isEnabled()
{
	...
	if constexpr (peripheral == Peripheral::Lcd)
		return RCC->APB1ENR1 & RCC_APB1ENR1_LCDEN;
	if constexpr (peripheral == Peripheral::Lptim1)
	...
	if constexpr (peripheral == Peripheral::Sai2)
		return RCC->APB2ENR & RCC_APB2ENR_SAI2EN;
	if constexpr (peripheral == Peripheral::Sdmmc1)
		return RCC->APB2ENR & RCC_APB2ENR_SDMMC1EN;
	if constexpr (peripheral == Peripheral::Spi1)
