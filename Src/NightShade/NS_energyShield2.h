...
{
public:
	NS_energyShield2(I2C_HandleTypeDef *pI2C);
	NS_energyShield2(I2C_HandleTypeDef *pI2C, uint16_t batteryCapacity_mAh);

	// RTC Functions
	void setTimeDate(uint8_t second, uint8_t minute, uint8_t hour,
...
private:
	void 	 writeByte(uint8_t slaveAddress, uint8_t registerAddress, uint8_t data);
	...
	bool 	 checkIfSealed(uint8_t slaveAddress);
	int 	 setupFuelGauge(uint8_t slaveAddress, uint16_t newDesignCapacity_mAh, uint16_t newTerminationVoltage_mV,
			uint16_t chargeTerminationCurrent_mA, uint8_t alarmSOC);
